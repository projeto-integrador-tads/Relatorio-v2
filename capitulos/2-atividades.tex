...
\begin{longtblr}[
	...
	{(FK)\\remetente}     & VARCHAR                & Chave estrangeira.~                                \\
	
	{(FK)\\viagens}       & VARCHAR                & Chave estrangeira.~                                 \\
	
	{(FK)\\destinatario}  & VARCHAR                & Chave estrangeira.~
...
\begin{longtblr}[
	...
	{(FK) \\enderecos\\\_destino} & VARCHAR & Chave estrangeira.\\
	
	{(FK) \\enderecos\\\_partida} & VARCHAR & Chave estrangeira.\\
...
A interface foi projetada com foco na responsividade, usabilidade e experiência do usuário, permitindo que esses, mesmo que em diferentes dispositivos móveis, tenham uma utilização intuitiva e fluida. O VemComigo é uma aplicação voltada para a mobilidade e deslocamento de pessoas, dito isso, imagina-se que o usuário final busca simplicidade para escolher rapidamente um meio viável de transporte em meio a agitação do dia a dia. Nesse contexto, decidiu-se então que as telas deveriam ser minimalistas de forma que apenas informações necessárias ocupassem espaço na tela, evitando confusões em usuários mais desatentos. As principais escolhas do desing foram:

\textbf{Paleta de Cores:} A escolha das cores seguiu princípios de acessibilidade e contraste, sendo composta por:
\begin{itemize}
	\item Cor principal: \#0064D2 (Azul vibrante) – representa confiança, tecnologia e mobilidade.
	\item Cor de Fundo: \#FFFFFF (Branco) – proporciona um ambiente limpo e moderno, facilitando a leitura e destacando os elementos interativos.
	\item Cores secundárias e de apoio: tons de cinza e azul escuro foram utilizados para equilibrar a interface e manter um visual neutro e sofisticado.
\end{itemize}
...
\textbf{Inputs, Botões e Icones:} Os campos de entrada (inputs) foram projetados para serem acessíveis e de fácil identificação. Foram aplicados bordas arredondadas, espaçamento adequado e feedback visual para indicar erros ou sucessos nas interações dos usuários. Os botões seguem um estilo minimalista, utilizando o azul primário (\#0064D2) para submeter ações principais. Adotou-se também uma biblioteca de ícones chamada ``Tabler'', disponível em [https://tabler.io/admin-template].

\textbf{Logo} O logotipo do aplicativo foi desenvolvido para refletir o conceito de transporte de maneira abstrata. Utilizou-se uma abordagem minimalista, incorporando elementos gráficos que remetem a um formato de uma roda.


\subsection{Componentização e Reutilização}

%Organização dos componentes. Estratégias para reutilização de código. Gerenciamento de estado (Redux, Context API, Zustand, etc.).
...
\textbf{Atores:}
\begin{itemize}
	\item Passageiro – Usuário que solicita caronas.
	\item Motorista – Usuário que oferece caronas.
	\item Administrador – Responsável por gerenciar o sistema.
\end{itemize}

\textbf{Casos de Uso:}
\begin{enumerate}
	\item O passageiro pode solicitar caronas, gerenciar reservas e avaliar motoristas.
	\item O motorista pode cadastrar veículos, oferecer corridas e interagir com passageiros.
	\item  O administrador, por sua vez, possui permissões para gerenciar usuários e monitorar avaliações.
\end{enumerate} 
